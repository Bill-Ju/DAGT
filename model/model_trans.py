...
class MaskedMultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, h, g):
        ...
        attention_weights = attention_weights * mask  # Apply mask
        output_in = torch.matmul(attention_weights, v)
        ...
        if self.dual_attention:
            ...
            output_out = torch.matmul(attention_weights_transposed, v)
    ...
    def forward(self, h, g):
        ...
        if self.dual_attention:
            ...
            if self.gates:
                ...
                gate_signal = F.dropout(self.gate_mlp(gate_input), self.dropout, training=self.training)
                gate_signal = self.gate_norm(gate_signal)
                gate_signal = F.dropout(F.sigmoid(gate_signal), self.dropout, training=self.training)
                output_out = gate_signal * output_out
